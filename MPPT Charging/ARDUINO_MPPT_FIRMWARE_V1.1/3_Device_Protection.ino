void backflowControl(){                                                //PV BACKFLOW CONTROL (INPUT MOSFET) 
  if(output_Mode==0){bypassEnable=1;}                                  //PSU MODE: Force backflow MOSFET on
    ...
    if(voltageInput>voltageOutput+voltageDropout){bypassEnable=1;}     //CHARGER MODE: Normal Condition -  MOSFET (on by default when not in MPPT charger mode)
  ...
  digitalWrite(backflow_BAT,!bypassEnable);     
}
